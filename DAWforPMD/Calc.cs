...
namespace DAWforPMD
{
    class Calc
    {
        ...
        public static float CalcML(Byte ML)
        {
            if (ML == 0) return 0.5f;
            else return ML;
        }

        public static float CalcDT(Byte DT, Byte Block, Byte Note)
        {
            float DTout = Default.Detune[DT < 4 ? DT : DT - 4, Block * 4 + Note] * (DT < 4 ? 1 : -1);
            return DTout;
        }

        public static float CalcFreq(Byte ML, Byte DT, Byte Block, Byte Note)
        {
            return 0f;
        }

        public static Byte CalcHex(int bit) //10000101とかで入力、出力は0x85など。
        {
            Byte HEXout = 0;
            for(int i = 0; i < 8; i++)
            {
